...
class HDF5DatasetWriterWithTokens:
    def __init__(self,
                 dims,
                 outputPath,
                 dataKey="images",
        ...
        self.tokens = self.db.create_dataset("tokens", (dims[0], tokenSize),
                                             dtype='int')

        # store the buffer size then initialise the buffer itself along with
        # the index into the datasets
        self.bufSize = bufSize
        self.buffer = {'data': [], 'labels': [], 'tokens': []}
        self.idx = 0

    # method to add data to BUFFER
    def add(self, rows, labels, tokens):
        # add the rows and labels to the buffer
        self.buffer["data"].extend(rows)
        self.buffer["labels"].extend(labels)
        self.buffer['tokens'].extend(tokens)
    ...
    def flush(self):
        ...
        self.tokens[self.idx:i] = self.buffer['tokens']
