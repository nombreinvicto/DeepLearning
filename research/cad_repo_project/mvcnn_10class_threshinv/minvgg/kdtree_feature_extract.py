...
def fit_kdtree(feature_matrix, sample_signature, k=50):
...
sample_signature = create_image_signature("sample_nut",
                                          cnn_model,
                                          normalise=True)

# now get closes distances and indices from kdtree - dists are sorted
dist, indices = fit_kdtree(features, sample_signature)

#%%

# get he neighbor names
# neighbor names are acc to sorted indices, these are unique names
indices_that_sort_indices = np.argsort(indices)
sorted_indices = np.sort(indices)
argsorted_dist = dist[indices_that_sort_indices]
neighbor_names = signature_db['label_names'][sorted_indices]
...
for path in image_paths:
    ...
    if image_name in list(neighbor_names):
        ...
        shutil.copy2(src=path, dst=f"{target_dir}//D={euclid_dist}.jpg")
#%%
